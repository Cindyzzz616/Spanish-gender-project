...
```{r eval=T, echo=T, message=F}
# Step 1: Mark existing rows as Trackloss = 0
# fix.dat$Trackloss <- 0
# or not bc we already have a Trackloss column that is all FALSE

# TODO figure out which participants did which trials

# Step 1: Create complete time series
all_times <- seq(
  min(fix.dat$TimeMS_adjusted),
  max(fix.dat$TimeMS_adjusted),
  by = 10
)
```
...
# Expand to include every timepoint for every combination
...
# NOTE could add other trial variables as needed
trial_info <- fix.dat %>%
  filter(!is.na(Determiner)) %>%
  group_by(TOI) %>%
  summarise(
    Determiner = unique(Determiner),
    # NOTE don't think SpeakerType should be included in trial_info - it's not a property of a trial
...
```{r eval=T, message=F}
# Check that everyone now has 743 bins
fix.dat.full %>%
  group_by(Participant, TOI) %>%
  summarise(n_bins = n()) %>%
  summarise(
    n_missing = sum(n_bins != 743),
...
```{r eval=T}
# Broad analysis window
WINDOW_START <- -300
WINDOW_END <- 1400

fix.dat.full <- fix.dat.full %>%
  filter(TimeMS_adjusted >= WINDOW_START, TimeMS_adjusted <= WINDOW_END)
```

##### Change `TOI` and `Participant` to factor and create a list of all trial IDs and participants
...
# right now, it should be 96 trials * 32 participants = 3072
# should actually be about half of this since half of the participants did half of the trials
fix.dat.full %>%
  distinct(Participant, TOI) %>%
  nrow()
```
...
df_plot <- fix.dat.full %>%
  filter(Participant == "TA105", TOI == "Trial01-def-campesina_0") %>%
...
ggplot(df_plot, aes(x = TimeMS_adjusted, y = Fixation, color = AOI)) +
  ...
  geom_vline(xintercept = 551, linetype = "dotted", color = "black") +
  theme_minimal()
