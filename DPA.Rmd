...
##### Re-bin the fixation data and store it in a new dataframe
...
# NOTE Should we take the time points to be the left edge, middle or right edge of the step function? Like how you'd take left-handed vs right-handed vs middle Riemann sums
...
##### UPDATE create a step function for the average FixP of all participants for each trial
...
calc_auc_interval <- function(df, start, end) {
  ...
  if (nrow(df_sub) < 2) { # NOTE why does it have to be nrow(df_sub) < 2?
    print("Empty subset for window ", start, "-", end, " (n = ", nrow(df_sub), ")")
    return(NA_real_)
  }
...
test_df %>%
  filter(TimeMS_adjusted >= 0, TimeMS_adjusted < 10)

calc_auc_interval(test_df, 310, 320)
```

##### Create test subsets

```{r eval=T, message=F}
# Create a test subset of fix.twovar with only one participant (and all the participant's trials)
...
##### Step 2: Compute AUCs from the stored step function
...
##### Step 3: Vectorize across many windows
