...
# Step 3: Merge original data with complete tifme series
fix.dat <- fix.dat %>%
...
#   group_by(Participant, TOI) %>%
#   mutate(
#     Bin = floor(TimeMS_adjusted/10+268))
#   ) %>%
#   ungroup()
```
...
##### NEW create a step function for the average FixP of all participants for each trial

```{r eval=T, message=F}
fix.twovar.mean <- fix.twovar %>%
  group_by(Participant, AOI, SpeakerType, Determiner, TimeMS_adjusted) %>%
