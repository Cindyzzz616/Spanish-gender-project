...
### UPDATE create a step function for the average FixP of all participants for each trial. Right now, we are stretching the portion between determiner onset (time=0) and noun offset (ending offset). We *will* need to do this twice eventually, splitting the time-course data at *noun onset*.
...
```{r}
auc_trial_windows <- step_functions %>%
  left_join(trial_windows, by = "TOI", relationship = "many-to-many") %>%
  mutate(
    AUC = pmap_dbl(
      list(f, start, end),
...
```{r eval=T, message=F}
auc_trial_windows <- auc_trial_windows %>%
  group_by(Participant, TOI) %>%
...
##### Compute average AUCs across participants (for smooth plotting)

```{r eval=T, message=F}
auc_summary <- auc_trial_windows %>%
  group_by(AOI, PBin) %>%
  summarise(
...
```{r eval=T, message=F}
ggplot(auc_summary, aes(x = PBin, y = mean_AUC_prop, color = AOI, group = AOI)) +
  geom_line(size = 1.2) +
