...
all_combos <- expand_grid(Participant = participants, TOI = TOIs)

# Expand to include every timepoint for every combination
full_template <- all_combos %>%
  tidyr::expand(Participant, TOI, TimeMS_adjusted = all_times)
```
...
trial_info <- fix.dat %>%
  ...
  summarise(
    ...
    NounOnset_adjusted = first(NounOnset_adjusted),
    EndingOffset_adjusted = first(EndingOffset_adjusted),
    .groups = "drop"
  )
...
fix.dat.full <- fix.dat.full %>%
  ...
  select(-any_of(c("Determiner.trial", "SpeakerType.trial",
                   "NounOnset_adjusted.trial", "EndingOffset_adjusted.trial")))
