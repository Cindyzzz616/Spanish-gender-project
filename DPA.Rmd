...
calc_auc_interval <- function(df, start, end) {
  # Create a subset of the dataframe based on start and end
  df_sub <- df %>%
    filter(TimeMS_adjusted >= start, TimeMS_adjusted < end)
  # If there are more than two time points remaining, calculate the AUC in that interval
  if (nrow(df_sub) < 2) return(NA_real_)
...
auc_trial_windows <- pmap_dfr( # Uses purrr::pmap_dfr to iterate row-by-row over the tibble trial_windows, feeding each row’s columns as arguments to the anonymous function. It row-binds (_dfr) all returned tibbles into one data frame.
  ...
  function(TOI, trial_duration, start, end) { # The per-row function—receives scalars for the current trial id, its duration, and the window bounds.
    if (!TOI %in% fix.twovar$TOI) return(tibble()) # Guard: if this trial id isn’t present in fix.twovar, skip this row by returning an empty tibble (which pmap_dfr will ignore).
    df_trial <- fix.twovar %>% filter(TOI == !!TOI) # The !! (bang-bang) “injects” the scalar TOI value into the filter so it compares the column TOI to the current trial id. df_trial is a dataframe with all the rows from a trial in the original dataset
    ...
    df_trial %>% # For the current trial’s rows...
      ...
      mutate(TOI = TOI, WindowStart = start, WindowEnd = end,
             WindowDur = end - start, AUC_prop = AUC / (end - start))
  }
)
