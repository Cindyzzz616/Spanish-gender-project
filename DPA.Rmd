...
TOIs <- as.character(unique(fix.dat$TOI))
...
for (trial_id in TOIs[1:5]) {
  # calculate duration of pseudo-bins for the given trial
  noun_onset <- unique(fix.dat$NounOnset_adjusted[fix.dat$TOI == trial_id])
  ending_offset <- unique(fix.dat$EndingOffset_adjusted[fix.dat$TOI == trial_id])
  pbin_dur = (ending_offset - noun_onset) / 100
  
  for (participant in participants[1:5]) {
    # select all the bins in the window of interest of the given trial and participant
    df_trial <- fix.dat[fix.dat$TOI == trial_id & 
                        fix.dat$Participant == participant &
                        fix.dat$NounOnset_adjusted + 200 <= fix.dat$TimeMS_adjusted &
                        fix.dat$TimeMS_adjusted <= fix.dat$EndingOffset_adjusted + 200, ]
    
    # unique_bins <- sort(unique(df_trial$Bin))
    
    # iterate through 100 intervals in the window of interest to create 100 pseudo-bins
    i = 1
    while (i <= 100) {
      ...
      bins_in_interval <- df_trial %>%
        filter(TimeMS_adjusted >= i * noun_onset, # TODO double check if this is supposed to be noun onset/ending offset or pbin_dur
               TimeMS_adjusted <= (i+1) * ending_offset) %>%
        pull(Bin) %>%
        unique() %>%
        sort()
      
      # iterate through each real bin in the interval
      for (bin in bins_in_interval) {
        
        # select a real bin in an interval with the length as the pseudo-bin duration
        ...
        # Example: compute mean fixation in this bin
        # mean_fix <- mean(df_bin$FixP_mean)
      }
      
      competitor_fixP = competitor_fix_count / pbin_dur # TODO double check this formula
