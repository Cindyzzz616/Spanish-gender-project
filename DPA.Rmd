...
```{r eval=T, echo=T, message=F}
fix.dat.full = fix.dat.full %>% mutate_at(vars(TOI, Participant, Determiner, SpeakerType, Phonol_Overlap_T), as.factor)
...
auc_summary_det <- auc_trial_windows_det %>%
  group_by(AOI, PBin, SpeakerType, Determiner) %>%
  summarise(
...
# 1. Shift noun bins
# Number of pseudobins (must match what you used earlier)
N_PBINS <- 1000
...
# Make sure grouping vars are factors (important for facet labels)
...
auc_plot <- ggplot(
  ...
  # Optional vertical line at noun onset
  geom_vline(xintercept = N_PBINS,
             linetype = "dashed",
             linewidth = 0.8) +
