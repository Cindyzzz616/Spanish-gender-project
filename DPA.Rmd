...
```{r eval=T, echo=T, message=F}
fix.dat = fix.dat %>% mutate_at(vars(TOI, Participant), as.factor)
...
ggplot(plot_df, aes(x = TimeMS_adjusted, y = FixP, color = AOI)) +
  ...
  scale_x_continuous(limits = c(305, 345)) +
...
##### Should we take the time points to be the left edge, middle or right edge of the step function? Like how you'd take left-handed vs right-handed vs middle Riemann sums
...
```{r eval=T, message=F}
# Define the number of pseudo-bins
Npbins = 100
win_start = 0
win_end = 1400

fix.twovar <- fix.twovar %>%
  filter(TimeMS_adjusted >= win_start, TimeMS_adjusted <= win_end)
...
trial_windows <- fix.twovar %>%
  ...
  mutate(
    window_list = map(trial_duration, function(dur) {
      edges <- seq(0, dur, length.out = Npbins + 1)
...
auc_trial_windows <- pmap_dfr(
  trial_windows,
  function(TOI, trial_duration, start, end) {
    if (!TOI %in% fix.twovar$TOI) return(tibble())
    df_trial <- fix.twovar %>% filter(TOI == "Trial14-pos-maestro_0")
    if (nrow(df_trial) == 0) return(tibble())

    df_trial %>%
      group_by(Participant, AOI) %>%
      summarise(AUC = calc_auc_interval(cur_data(), start, end), .groups = "drop") %>%
